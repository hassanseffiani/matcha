...
connection.connect(function (err) {
  ...
  connection.query(`CREATE DATABASE Matcha`, (err) => {
    if (err);
      ...
      connection.query(
        'CREATE TABLE IF NOT EXISTS conversations (`id` int(11) NOT NULL PRIMARY KEY AUTO_INCREMENT, `id_from` int(11) NOT NULL, `id_to` int(11) NOT NULL, `content` VARCHAR(255) NOT NULL, `timestamp` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP)'
      )
      connection.query(
        `CREATE PROCEDURE delete_like() BEGIN DELETE FROM blocked WHERE HOUR(TIMEDIFF(created_at, now())) >= 5 AND dlt = 1; END`
      )
      connection.query(
        `CREATE PROCEDURE Update_duplicated_userName() BEGIN UPDATE users SET userName = concat(userName,'0') WHERE oauth_id IS NOT NULL AND users.userName in (SELECT * FROM (SELECT userName FROM users GROUP BY userName HAVING COUNT(userName) > 1) as a); END`
      )
      connection.query(
        `CREATE EVENT myevent ON SCHEDULE EVERY 1 SECOND DO CALL delete_like();`
      )
      connection.query(
        `CREATE EVENT myevent1 ON SCHEDULE EVERY 1 SECOND DO CALL Update_duplicated_userName();`
      )
